...
/// Frozen snapshot of the schema as it stood when calculation depth
/// shipped: V4 plus the custom-angle, Isha-rule, and per-prayer-offset
/// fields on `UserSettings`. These nested copies must never change —
/// they exist so the migration plan can describe a V5 store exactly,
/// including the stores a build between the calculation work and the
/// sound work already created. The live model classes are top-level in
/// `Models/` and are listed by the latest `IhsanSchemaV*`.
