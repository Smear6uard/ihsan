...
/// Frozen snapshot of the schema as it stood when the sunnah layer
/// shipped: V2 plus the voluntary-worship record (`NaflLog`) and the
/// defaulted sunnah-layer / night-wake preference fields on
/// `UserSettings`. These nested copies must never change — they exist
/// so the migration plan can describe a V3 store exactly. The live
/// model classes are top-level in `Models/` and are listed by the
/// latest `IhsanSchemaV*`.
public enum IhsanSchemaV3: VersionedSchema {
    ...
    public final class PrayerLog {
        ...
        public init(
            ...
            self.dedupKey = "\(prayer.rawValue)-\(formatter.string(from: prayerDate))"
    ...
    public final class NaflLog {
        ...
        public init(
            ...
            self.id = id
            self.dedupKey = IhsanCore.NaflLog.makeDedupKey(kind: kind, naflDate: naflDate)
